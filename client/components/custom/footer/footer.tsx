...
export default function Footer() {
  ...
  return (
    <footer className="w-full border-t bg-muted/20 pb-12 pt-16 transition-colors duration-300">
      <div className="container px-6">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-12 md:gap-8">
          ...
          <div className="flex flex-col items-start gap-4 md:col-span-5">
            ...
            <p className="max-w-sm text-sm leading-relaxed text-muted-foreground">
              In an age of digital noise, we curate space for clarity, focus, and profound writing. Rebuild your connection to deep thoughts.
            </p>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
              <div className="flex items-center gap-2 rounded-full border bg-background/50 px-3 py-1.5 backdrop-blur-xs">
                ...
                <span>Session focus: <span className="font-mono font-medium text-foreground">{formatTime(seconds)}</span></span>
              </div>
            ...
            <ul className="flex flex-col gap-2.5 text-sm">
              {NAV.map((item: NavType, idx: number) => (
                <li key={idx}>
                  <Link 
                    href={item.url} 
                    className="inline-block text-muted-foreground hover:text-foreground hover:underline underline-offset-4 transition-colors duration-200"
                  >
                    {item.name}
                  </Link>
                </li>
          ...
          <div className="flex flex-col gap-3 sm:col-span-4 md:col-span-2">
            ...
            <ul className="flex flex-col gap-2.5 text-sm">
              <li>
                <Link href="/write" className="inline-block text-muted-foreground hover:text-foreground hover:underline underline-offset-4 transition-colors duration-200">
                  Write a Story
                </Link>
              </li>
              <li>
                <Link href="/profile" className="inline-block text-muted-foreground hover:text-foreground hover:underline underline-offset-4 transition-colors duration-200">
                  Profile
                </Link>
              </li>
              <li>
                <Link href="/settings" className="inline-block text-muted-foreground hover:text-foreground hover:underline underline-offset-4 transition-colors duration-200">
                  Settings
                </Link>
              </li>
            </ul>
                  ...
                  >
                    <div className="relative flex w-full items-center">
                      ...
                      <button
                        type="submit"
                        disabled={subStatus === "loading"}
                        className="absolute right-2 flex h-7 w-7 items-center justify-center rounded-md hover:bg-muted transition-colors duration-200"
                      >
                        {subStatus === "loading" ? (
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-6 border-t pt-8 md:flex-row">
          <p className="text-xs text-muted-foreground">
            &copy; {new Date().getFullYear()} kata. All rights reserved. Build by <Link href={'https://gedeapriana.vercel.app'}>@gedeapriana</Link>
          </p>
