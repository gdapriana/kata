...
export class ImageController {
    ...
    try {
      ...
      if (!file) {
        throw new ResponseError(
          ErrorResponseMessage.BAD_REQUEST("No file uploaded"),
        );
      }
  ...
  static UploadBulk = async (
    ...
    try {
      ...
      if (!files || files.length === 0) {
        throw new ResponseError(
          ErrorResponseMessage.BAD_REQUEST("No files uploaded"),
        );
      }
  ...
  static Delete = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!id) {
        throw new ResponseError(
          ErrorResponseMessage.BAD_REQUEST("Image id is required"),
        );
      }
