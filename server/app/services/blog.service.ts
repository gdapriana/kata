...
export class BlogService {
  static GetOne = async (
    ...
    BlogGetOneResponseType & { liked?: boolean; bookmarked?: boolean }
  > => {
  ...
  static GetSaved = async (data: BlogValidationGetAll, userId: string) => {
  ...
  static GetLiked = async (data: BlogValidationGetAll, userId: string) => {
    return blogGetAll(data, userId, {
      likedByUsers: {
        some: {
          id: userId,
  ...
  static Create = async (data: BlogValidationCreate, authorId: string) => {
    ...
    if (!category) {
      throw new ResponseError(
        ErrorResponseMessage.NOT_FOUND("category" as any),
      );
    }
    ...
    if (existingBlog) {
      throw new ResponseError(
        ErrorResponseMessage.ALREADY_EXISTS("blog" as any),
      );
    }
      ...
      data: {
        ...
        tags:
          data.tags && data.tags.length > 0
            ...
            : undefined,
        galleryImages:
          data.galleryImageIds && data.galleryImageIds.length > 0
            ? {
  ...
  static Delete = async (id: string, userId: string, userRole: string) => {
    const blog = await prismaClient.blog.findUnique({
      where: { id },
